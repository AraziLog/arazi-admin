    ...
    <div style={{ minHeight:'100vh', background:'var(--sidebar-bg)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      ...
      <div style={{ width:420, background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:20, padding:40, backdropFilter:'blur(20px)', boxShadow:'0 25px 50px rgba(0,0,0,.4)', position:'relative', animation:'fadeUp .4s ease' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:32 }}>
          <div style={{ width:80, height:80, borderRadius:16, margin:'0 auto 16px', background:'rgba(255,255,255,.08)', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
            <img
              src={LOGO_URL}
              alt="ARAZI Logistics"
        ...
        <form onSubmit={handleLogin}>
          {[
            ['Username or Email', username, setUsername, 'text',     'admin'],
            ['Password',          password, setPassword, 'password', '••••••••'],
          ].map(([label, val, setter, type, ph]) => (
            <div key={label} style={{ marginBottom:14 }}>
              <label style={{ display:'block', fontSize:12, fontWeight:600, color:'rgba(255,255,255,.5)', marginBottom:6 }}>
                {label}
              </label>
              <input
                type={type}
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ width:'100%', padding:13, borderRadius:8, background:'linear-gradient(135deg,#1a56db,#1e40af)', color:'#fff', border:'none', fontSize:15, fontWeight:700, cursor:'pointer', marginTop:4, opacity: loading ? .7 : 1 }}
          >
            {loading ? 'Signing in…' : 'Sign In'}
          </button>

          <div style={{ textAlign:'center', marginTop:16, fontSize:11, color:'rgba(255,255,255,.25)' }}>
            Demo: enter any credentials to preview the dashboard
          </div>
        </form>

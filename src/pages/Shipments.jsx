...
import { fetchShipments, syncShipment } from '../services/api'
...
export default function Shipments({ onNavigate }) {
  ...
  const loadShipments = () => {
    ...
    fetchShipments()
      ...
      .catch(err  => { setError(err.message || 'Failed to load shipments'); setLoading(false) })
  }
  ...
  const filtered = shipments.filter(s => {
    ...
    const matchSearch = !search || [s.tracking_number, s.customer_name, s.origin, s.destination]
  ...
  const handleSync = async (id) => {
    setSyncing(id)
    try { await syncShipment(id); notify('Carrier sync complete'); loadShipments() }
    ...
    <div style={{ animation:'fadeUp .3s ease' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
        <div>
          ...
          <p style={{ color:'var(--text-3)', fontSize:13, marginTop:4 }}>{loading ? 'Loading…' : `${filtered.length} shipments found`}</p>
        </div>
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={loadShipments} style={{ background:'var(--surface-2)', color:'var(--text-2)', border:'1px solid var(--border)', padding:'10px 16px', borderRadius:'var(--radius)', fontSize:13, fontWeight:600, cursor:'pointer' }}>↻ Refresh</button>
      ...
      {error && (
        <div style={{ padding:'14px 18px', background:'var(--danger-soft)', border:'1px solid #fca5a5', borderRadius:'var(--radius)', marginBottom:20, fontSize:13, color:'var(--danger)' }}>
          ✗ {error} — Make sure the Arazi plugin is active on WordPress and your Application Password is valid.
        </div>
      )}
      ...
      <div className="card" style={{ overflow:'hidden' }}>
        {loading ? (
          <div style={{ padding:48, textAlign:'center', color:'var(--text-3)' }}>
            <div style={{ width:32, height:32, border:'3px solid var(--border)', borderTopColor:'var(--primary)', borderRadius:'50%', animation:'arazi-spin .7s linear infinite', margin:'0 auto 12px' }}/>
            Loading shipments from WordPress…
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map(s=>(
                  <tr key={s.id} style={{ borderBottom:'1px solid var(--border-light)', cursor:'pointer' }} onMouseEnter={e=>e.currentTarget.style.background='var(--surface-2)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                    ...
                    <td style={{ padding:'12px 16px' }}><span style={{ padding:'4px 12px', borderRadius:99, fontSize:11, fontWeight:700, background:STATUS_COLOR[s.status]+'18', color:STATUS_COLOR[s.status] }}>{s.status_label||STATUS_LABEL[s.status]}</span></td>
                    <td style={{ padding:'12px 16px', color:'var(--text-2)' }}>{s.weight_kg ? s.weight_kg+' kg' : '—'}</td>
                ...
                {filtered.length===0&&(
                  <tr><td colSpan={8} style={{ padding:48, textAlign:'center', color:'var(--text-3)' }}>
                    ...
                    <div style={{ fontWeight:700, marginBottom:6 }}>No shipments yet</div>
                    <div style={{ fontSize:12, marginBottom:16 }}>Create your first shipment to get started</div>
                    <button onClick={()=>onNavigate('addshipment')} style={{ background:'var(--primary)', color:'#fff', border:'none', padding:'10px 20px', borderRadius:'var(--radius)', fontSize:13, fontWeight:600, cursor:'pointer' }}>+ Add Shipment</button>
                  </td></tr>
                )}
              </tbody>
      ...
      {selected && <ShipModal s={selected} onClose={()=>setSelected(null)} onNotify={notify} onRefresh={loadShipments}/>}
...
function ShipModal({s, onClose, onNotify, onRefresh}) {
      ...
      <div style={{ background:'var(--surface)', borderRadius:'var(--radius-lg)', width:560, maxWidth:'95vw', maxHeight:'85vh', overflow:'auto', boxShadow:'var(--shadow-lg)', animation:'fadeUp .2s ease' }}>
        <div style={{ padding:'20px 24px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center', background:'var(--sidebar-bg)', borderRadius:'var(--radius-lg) var(--radius-lg) 0 0' }}>
          ...
          <div style={{ display:'flex', gap:12 }}>
            <span style={{ padding:'6px 14px', borderRadius:99, fontSize:12, fontWeight:700, background:STATUS_COLOR[s.status]+'30', color:STATUS_COLOR[s.status] }}>{s.status_label||STATUS_LABEL[s.status]}</span>
        ...
        <div style={{ padding:24 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:20 }}>
            {[['Customer',s.customer_name],['Email',s.customer_email],['Service',s.service_type],['Weight',s.weight_kg?s.weight_kg+' kg':'—'],['Est. Delivery',s.estimated_delivery||'—'],['Created',new Date(s.created_at).toLocaleDateString()]].map(([l,v])=>(
          ...
          <div style={{ borderTop:'1px solid var(--border)', paddingTop:18 }}>
            ...
            <button onClick={()=>{onNotify('Shipment updated');onRefresh();onClose()}} style={{ background:'var(--primary)', color:'#fff', border:'none', padding:'10px 24px', borderRadius:'var(--radius)', fontSize:13, fontWeight:600, cursor:'pointer' }}>Save Changes</button>
          </div>
        </div>
      </div>
    </div>
